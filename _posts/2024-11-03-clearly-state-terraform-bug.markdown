...
Today (_09:02 Sunday 3rd Nov 2024_) there are 3,428 open issues in the terraform-aws-provider repository. There are 393 open pull-requests.
...
The end we are hoping for is a merged bugfix, the first step in that direction is a clear and accurate bug report.

### What really happened

It can be tricky to state things clearly and simply. We all want to provide lots of information we think will be useful. I include myself in the group that often over-complicates explanations.
...
I often write down what happened, and then revise the wording away from a computer in a more human environment. In a cafe, with a coffee, I try and rewrite using the simplest words and sentences that accurrately describe what happened. Remember, not everyone’s first language is the same as yours.

The project maintainers want you to report the behaviour experienced, not opinions or solutions (at this point). They are trying to operate efficiently by standardising issue reporting.
...
From reading through the issues, it seems that this is a long-standing problem affecting quite a few people. The number of 👍 on issues and comments indicates how many others have the same problem.

There are workarounds given for this problem (editing statefile, manual deletions, etc), but I don’t see any proposed fix. Over eleven years, no-one has submitted a PR as far as I can see. The first issue remains open, the others have been closed due to inactivity.
